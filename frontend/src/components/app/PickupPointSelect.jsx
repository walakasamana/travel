...
// INV-REF-02 batch 2: titik jemput = relasi ke master `pickup_points`.
// Quick-add menulis ke MASTER (satu pintu) — bukan menyelundupkan teks bebas ke booking.
export const PickupPointSelect = ({ value, onChange, testId = "origin-select" }) => {
  const [options, setOptions] = useState([]);
  const [adding, setAdding] = useState(false);
  const [newName, setNewName] = useState("");

  const load = useCallback(() => {
    apiClient.get("/pickup-points")
  ...
  const quickAdd = async () => {
    ...
    try {
      const { data } = await apiClient.post("/pickup-points", { name });
      toast.success(`Titik jemput "${data.name}" siap dipakai`);
      setNewName(""); setAdding(false);
      load(); onChange(data.name);
    } catch (e) {
      toast.error(e?.response?.data?.detail || "Gagal menambah titik jemput");
    }
  };
  ...
  return (
    <div className="space-y-1.5">
      <Select value={value || ""} onValueChange={onChange}>
        <SelectTrigger data-testid={testId}><SelectValue placeholder="Pilih titik jemput" /></SelectTrigger>
        <SelectContent>
      ...
      {adding ? (
        <div className="flex gap-1.5">
          <Input className="!h-8 text-[12px]" value={newName} onChange={(e) => setNewName(e.target.value)}
            placeholder="Nama titik jemput baru" data-testid={`${testId}-new-input`} />
        ...
        <button type="button" className="flex items-center gap-1 text-[11.5px] font-semibold text-[#007AFF]"
          ...
          <Plus size={12} /> Tambah titik jemput baru (masuk master)
        </button>
      )}
    </div>
  );
